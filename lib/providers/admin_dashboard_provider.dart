...
/// Holds the Admin dashboard's platform-wide statistics.
///
/// Kept entirely separate from a future `AdminUsersProvider`/
/// `AdminOrganizationsProvider`/`AdminSkillsProvider` — Dashboard, Users,
/// Organizations, and Skills are four independent resources, the same
/// separation already applied throughout this app's other provider pairs
/// (see `OrganizationAssessmentProvider` vs `OrganizationApplicationsProvider`).
class AdminDashboardProvider extends ChangeNotifier {
  ...
  /// here (unlike `OrganizationAssessmentProvider.loadForApplication`)
  /// because there's only ever one dashboard to load, not one per some ID.
  Future<void>? _pendingLoad;

  void _handleAuthChanged() {
    // A different user may sign in next — don't leak the previous admin
  ...
  Future<void> _performLoad() async {
    ...
    try {
      ...
      // blank out statistics the admin is already looking at.
